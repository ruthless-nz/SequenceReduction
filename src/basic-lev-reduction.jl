...
"""
basic_lev(dataframe, sequence_id="sequence_id", sequence_event="sequence_event", sequence_order="sequence_order")

Takes a dataframe of sequences,(type of DataFrames) that has the following three columns:
#sequence_id - the sequence identifier, this might be client, game, or transaction. 
#sequence_event - the event or data that is tied to the id, this might be 'new client', 'goal scored' or 'transaction approved'
#sequence_order - the order that the events happened in. this can be any ascending real number. 
